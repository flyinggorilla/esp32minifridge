...
static char tag[] = "Esp32MiniFridgeWebServer";

//------------------------------------------------------------------

Esp32MiniFridgeWebServer::Esp32MiniFridgeWebServer()
{
	mbRestart = false;
}
...
bool Esp32MiniFridgeWebServer::StartWebServer()
{
	...
	__uint16_t port;
	if (esp32minifridge.GetConfig().muWebServerPort)
		port = esp32minifridge.GetConfig().muWebServerPort;
	else
		port = esp32minifridge.GetConfig().mbWebServerUseSsl ? 443 : 80;

	return Start(port, esp32minifridge.GetConfig().mbWebServerUseSsl, &(esp32minifridge.GetConfig().msWebServerCert));
}

bool Esp32MiniFridgeWebServer::HandleRequest(HttpRequestParser &httpParser, HttpResponse &httpResponse)
{
