...
unsigned short WebClient::HttpExecute() {
	...
	// Socket
	int webClientSocket = socket(res->ai_family, res->ai_socktype, 0);
	if (webClientSocket < 0) {
	...
	// CONNECT
	if (connect(webClientSocket, res->ai_addr, res->ai_addrlen) != 0) {
	...
	if (mpPostData) {
		if (write(webClientSocket, mpPostData, muPostDataSize) < 0) {
	...
	while (!mHttpResponseParser.ResponseFinished()) {
		size_t sizeRead = read(webClientSocket, (char*)sReceiveBuf.c_str(), sReceiveBuf.length());
